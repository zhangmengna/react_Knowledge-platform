...
class Particular extends Component {
    ...
    searchEvent = () => {
        //查询
        window.Fetch(urlBefore + '/jcxx/querylist_icd9.action', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
    ...
    //删除
    delete = (id) => {
        window.Fetch(urlBefore + '/jcxx/delete_icd9.action', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
                ...
                render: (text, record) => (
                    <div style={{color: '#007cfd'}}>
                        <span className={style.cursor} title="修改"
                              onClick={() => this.amend(record.icd9Id)}>修改</span>&nbsp;|&nbsp;
                        <Popconfirm title="是否确认删除?" onConfirm={() => this.delete(record.icd9Id)}>
