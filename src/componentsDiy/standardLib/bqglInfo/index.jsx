...
class Particular extends Component {
    ...
    menuClick = (e) => {
        if (e.key === '1') {
            ...
            if (this.state.selectedRows.length > 0) {
                const obj = [];
                this.state.selectedRows.forEach((sel, i) => {
                    obj.push({
                        tagId: sel.tagId
                    })
                })
                // 分享标签信息
                window.Fetch(urlBefore + '/base/share_tags.action', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    },
    ...
    searchEvent = () => {
        //查询
        window.Fetch(urlBefore + '/base/querylist_tags.action', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
    ...
    //删除
    delete = (id) => {
        window.Fetch(urlBefore + '/base/delete_tags.action', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
    ...
    render() {
                ...
                render: (text, record) => (
                    <div style={{color: '#007cfd'}}>
                        <span className={style.cursor} title="修改"
                              onClick={() => this.amend(record.tagId)}>修改</span>&nbsp;|&nbsp;
                        <Popconfirm title="是否确认删除?" onConfirm={() => this.delete(record.tagId)}>
        ...
        const menu = <Menu style={{width: 170}} onClick={this.menuClick}>
            ...
            <Menu.Item key="2"> 分享标签 </Menu.Item>
        </Menu>
