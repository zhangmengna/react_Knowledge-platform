...
class Particular extends Component {
    ...
    searchEvent = () => {
        //查询
        window.Fetch(urlBefore + '/jcxx/querylist_symptomSigns.action', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
    ...
    //删除
    delete = (id) => {
        window.Fetch(urlBefore + '/jcxx/delete_symptomSigns.action', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
                ...
                render: (text, record) => (
                    <div style={{color: '#007cfd'}}>
                        <span className={style.cursor} title="修改"
                              onClick={() => this.amend(record.symptonId)}>修改</span>&nbsp;|&nbsp;
                        <Popconfirm title="是否确认删除?" onConfirm={() => this.delete(record.symptonId)}>
                ...
                <div className={this.state.pageShow ? "" : "hidden"}>
                    ...
                    <div className={style.header}>
                        ...
                        <div className={style.tableZone}>
                            <Table
                                bordered
                                columns={this.state.libType === "dz" ? column : columns}
                                rowSelection={rowSelection}
                                dataSource={this.state.data}
                                rowKey={record => record.symptonId}
